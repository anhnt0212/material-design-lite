...
| `getItems() => HTMLCollection` | Returns the list of elements inside the items container. |
| `getItemYParams(el: Element) => {top: number, height: number}` | Returns an object with the offset top and offset height values for the provided element. |
| `setTransitionDelay(el: Element, value: string) => void` | Sets the transition delay on the provided element to the provided value. |
