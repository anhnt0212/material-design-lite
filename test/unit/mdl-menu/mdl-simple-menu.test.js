...
test('adapter#setScale sets the correct transform on the menu element', t => {
  ...
  t.equal(root.style.getPropertyValue(getTransformPropertyName()), 'scale(0.42, 0.84)');
  t.end();
});
...
test('adapter#setInnerScale sets the correct transform on the items container', t => {
  ...
  t.equal(items.style.getPropertyValue(getTransformPropertyName()), 'scale(0.42, 0.84)');
  t.end();
});

test('adapter#getItems returns the list of children of the items container', t => {
  const {root, component} = setupTest();
  const items = root.querySelector(strings.ITEMS_SELECTOR);
  t.equal(component.getDefaultFoundation().adapter_.getItems(), items.children);
  t.end();
});

test('adapter#getItemYParams returns the height and top of the provided element', t => {
  const {component} = setupTest();
  const mockItem = {offsetTop: 42, offsetHeight: 84};
  t.deepEqual(component.getDefaultFoundation().adapter_.getItemYParams(mockItem), {top: 42, height: 84});
  t.end();
});

test('adapter#setTransitionDelay sets the correct transition-delay on the provided element', t => {
  const {component} = setupTest();
  const mockItem = bel`<a class="mdl-list-item" href="#">Item</a>`;
  component.getDefaultFoundation().adapter_.setTransitionDelay(mockItem, '0.42s');
  t.equal(mockItem.style.transitionDelay, '0.42s');
  t.end();
});
